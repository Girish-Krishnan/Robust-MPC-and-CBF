...
class DroneEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def is_healthy(self):
        ...
        for i in range(self.data.ncon):
            ...
            if geom2 == self.car_body_array[0]:
                if geom1 in self.drone_blade_array:
                    is_healthy = False
    ...
    def step(self, action):
        ...
        for i in range(self.data.ncon):
            ...
            if geom1 == self.car_body_array[0]:
                if geom2 in self.drone_body_array:
                    Touched_landing_box_set.add(str(self.sim.model.geom_id2name(sim_contact.geom1)))
    ...
    def _get_obs(self):
        # Get the camera ID for the drone’s onboard camera
        cam_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_CAMERA, "Tracking_com")

        if cam_id == -1:
            raise ValueError("Camera 'drone_cam' not found in the MuJoCo model.")

        # Set up the renderer
        width, height = 148, 148
        renderer = mujoco.Renderer(self.model, width, height)

        # Ensure the latest state is rendered
        mujoco.mj_forward(self.model, self.data)

        # Set the camera ID in the MuJoCo scene
        renderer.update_scene(self.data, camera=cam_id)  # Use `camera=cam_id` to render from the correct camera
        ...
        CHW = np.transpose(rgb_img[::-1, :, :], (2, 0, 1))
        # flip the image vertically
        CHW = np.flip(CHW, 1)

        # Observation dictionary
        obs_dct = {
            'image': np.array(CHW) / 255.0,  # Normalize pixels
            'vector': np.concatenate([self.action_buffer, [self.state_vector()[4]]])
        }
    ...
    def viewer_setup(self):
        if hasattr(self, "viewer") and self.viewer is not None:
            ...
            mujoco.mjv_defaultCamera(self.cam)  # Reset to default camera settings

            for key, value in DEFAULT_CAMERA_CONFIG.items():
                if hasattr(self.cam, key):
                    setattr(self.cam, key, value)

            self.viewer.update_scene(self.data)  # Ensure the scene updates with new camera settings
